...
毎回 http からcsvファイルをダウンロードすると時間がかかるので，PCにファイルを保存しておき，それを読み込むこと推奨。
...
email_data <- read_csv("d://Kevin_Hillstrom_MineThatData_E-MailAnalytics_DataMiningChallenge_2008.03.20.csv")

# 以下での説明簡略化のため行番号の追加 + 必要な列のみ抽出（実際の分析時には不要）
...
つまり，「メール未配信グループは優良顧客の割合を減らす」，「メール配信グループは優良顧客の割合を増やす」操作を行っている。乱数を使って削除するかどうかを決めているため，該当するデータをきっちり半分だけ削除しているわけではない。  
</br>

#### **乱数の追加**

`runif` で0～1の間の値を取る一様乱数を発生
...
- メール未配信グループ (`treatment == 0`) ⇒ 優良顧客の割合を減らす
  - 非優良顧客のデータ (`loyal == 0`) は全て残す 
  - 優良顧客のデータ (`loyal == 1`) で乱数が 0.5 未満 (`random_number < 0.5`) なら残し，乱数が 0.5 以上なら削除  


- メール配信グループ (`treatment == 1`) ⇒ 優良顧客の割合を増やす
  - 非優良顧客のデータ (`loyal == 0`) で乱数が 0.5 未満 (`random_number < 0.5`) なら残し，乱数が 0.5 以上なら削除
  - 優良顧客のデータ (`loyal == 1`) は全て残す  

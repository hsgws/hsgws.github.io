...
ただし，以下のGUIによるデータ読み込み方法はデータのファイルサイズが大きい場合に処理時間がかかる。ファイルサイズが数十メガバイト以上なら `read.table` 関数によるデータ読み込みを推奨。
...
### 2.4. パッケージのインストール
右下のペインの Packages から ![](fig/rstudio/package_install.png) ボタンをクリックし，インストールしたいパッケージ名を入力して Install をクリックする。

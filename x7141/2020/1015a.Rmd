---
title: "ロジットモデル（10/15）"
output:
  html_document:
    toc: true
    toc_float:
      collapsed: false
    df_print: "paged"
    highlight: pygments
    # theme: lumen
    md_extensions: -ascii_identifiers
    css: "template/style_ch.css"
    includes:
      in_header: "template/head.html"
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)

# packages
library(plotly)
library(broom)
# library(pander)
library(knitr)

# data
data_email <- read.csv("data/data_email.csv")
sec4_choice2 <- read.csv("data/sec4_choice2.csv")
```

## 1. 分析例①：二項ロジット
### データ
**メールマーケティングの配信データ**（データの一部のみ表示）

- 説明
- 説明


```{r, echo=FALSE}
head(data_email)
```

| 変数名          | 内容                                                         |
| --------------- | ------------------------------------------------------------ |
| **recency**         | 最終購買からの経過月数                                       |
| history_segment | 昨年の購買金額の階級                                         |
| **history**         | 昨年の購入金額                                               |
| mens            | 昨年に男性向け商品を購入しているか（1＝購入，0＝非購入）     |
| womens          | 昨年に女性向け商品を購入しているか（1＝購入，0＝非購入）     |
| zip_code        | 居住地区の分類（Rural，Surburban，Urban）                    |
| **newbie**          | 過去12ヶ月以内の新規顧客（1＝新規顧客，0＝それ以外）         |
| channel         | 昨年の購買チャネル（Multichannel，Phone，Web）               |
| **segment**         | 配信したプロモーションメールの種類 <br>（"Mens Email"＝男性向け，"Womens  Email"＝女性向け，"No Email"＝配信無し） |
| **visit**           | メール配信後2週間以内のサイト訪問の有無（1＝訪問あり，0＝訪問なし） |
| conversion      | メール配信後2週間以内の商品購買の有無（1＝購買あり，0＝購買なし） |
| spend           | 購入金額                                                     |
（太字は以下の分析例で使用する変数）


**データ出典** ： [MineThatData E-Mail Analytics And Data Mining Challenge dataset](https://blog.minethatdata.com/2008/03/minethatdata-e-mail-analytics-and-data.html)


### モデル


### Rコード
```{r, results='hide'}
# データ data_email は "Import Dataset" で読込済みとする
# ロジットモデルの実行
result <- glm(visit ~ recency + log(history) + segment + newbie,
              family = "binomial", data = data_email)

# 推定結果の表示
summary(result)
```

**`glm(formula, family, data)` ：二項ロジットモデルの実行コマンド**

- **`formula`**：目的変数・説明変数の指定
  - 「目的変数 ~ 説明変数1 + 説明変数2 + 説明変数3」のように，`~` の左側に目的変数，右側に説明変数を記述する
  - 説明変数が2個以上ある場合は，`+` で追加する
  
- **`family`**：分析モデル（データが従う確率分布）の指定
  - 二項ロジットの場合はデータが二項分布に従うので `"binomial"` を指定
  - `family` を変更すれば，ポアソン回帰などのその他の一般化線型モデルも実行可能

- **`data`**：分析データの指定  

##### Rコードの出力
```{r, echo=FALSE}
result %>% 
  summary() %>% 
  print()
```

### 推定結果
```{r, echo=FALSE}
result %>% 
  tidy() %>% 
  kable(format = "markdown",
        digits = 3,
        col.names = c("", "回帰係数", "標準誤差", "z 値", "p 値"))
```


## 2. 分析例②：多項ロジット*

### データ
**醤油3製品の選択データ**（データの一部のみ表示）

- 説明
- 説明

```{r, echo=FALSE}
head(sec4_choice2)
```

| 変数名    | 内容                                              |
| --------- | ------------------------------------------------- |
| id        | 消費者ID                                          |
| Date      | 購買日                                            |
| Choice    | 購買ブランド（A，B，C）                           |
| Price.A   | ブランドAの価格掛率（定価＝1）                    |
| Price.B   | ブランドBの価格掛率                               |
| Price.C   | ブランドCの価格掛率                               |
| Display.A | ブランドAの山積み陳列の有無（1＝実施，0＝非実施） |
| Display.B | ブランドBの山積み陳列の有無                       |
| Display.C | ブランドCの山積み陳列の有無                       |
| Ad.A      | ブランドAのチラシ掲載の有無（1＝実施，0＝非実施） |
| Ad.B      | ブランドBのチラシ掲載の有無                       |
| Ad.C      | ブランドCのチラシ掲載の有無                       |

**データ出典** ： 佐藤 (2016) 『マーケティングの統計モデル』朝倉書店（mlogitパッケージで分析するためデータの一部を変更）


### モデル


### Rコード
```{r, results='hide', message=FALSE}
# データ data_email は "Import Dataset" で読込済みとする
# mlogit パッケージの読込み
library(mlogit)

# mlogit 関数用にデータを加工
soydata <- mlogit.data(sec4_choice2, choice = "Choice", shape = "wide", varying = 4:12, sep = ".")

# 多項ロジットモデルの推定
result <- mlogit(Choice ~ Price + Display + Ad, data = soydata)

# 推定結果の表示
summary(result)
```


**`mlogit.data(data, choice, shape, varying, sep)`：mlogit パッケージ用にデータを変換**

- **`data`**：
- **`choice`**：
- **`shape`**：
- **`varying`**：
- **`seq`**：

**`mlogit(formula, data)`：多項ロジットモデルの実行**

- **`formula`**：目的変数・説明変数の指定
  - 「目的変数 ~ 説明変数1 + 説明変数2 + 説明変数3」のように，`~` の左側に目的変数，右側に説明変数を記述する
  - 説明変数が2個以上ある場合は，`+` で追加する

- **`data`**：`mlogit.data` コマンドで変換したデータの指定  


##### Rコードの出力
```{r, echo=FALSE}
result %>% 
  summary() %>% 
  print()
```


### 推定結果
```{r, echo=FALSE}
coef <- result$coefficients
std.err <- sqrt(-diag(solve(result$hessian)))
zvalue <- coef/std.err
pvalue <- pnorm(-abs(zvalue)) + (1-pnorm(abs(zvalue)))
result.df <- data.frame(coef, std.err, coef/std.err, pvalue) %>% 
  kable(format = "markdown",
        digits = 3,
        col.names = c("回帰係数", "標準誤差", "z 値", "p 値"))
result.df
```


## 3. レポート課題
### データ
二項なら分析例と同じデータ，多項ならケチャップデータ

二項で conversion を目的変数

### 課題



...
なお，ダミー変数については，変数が0から1に変化すると選択確率が何倍になるかをオッズ比で評価することができる。オッズ比は $\exp(\beta)$ で計算することができ，newbie の場合，$\exp(\beta_3) = \exp(-0.503) = 	0.605$ となり，新規顧客はそれ以外の既存顧客に比べて，プロモーションメールへの反応確率が 0.605 倍となる。
...
# 多項ロジットモデルの推定
result <- mlogit(Choice ~ log(Price) + Display + Ad, data = soydata, reflevel = "C")
...
- **`reflevel`**：切片を0に基準化する選択肢の指定（省略可能）
...
マーケティング変数が選択確率に与える影響としては，期待通り，価格掛率はマイナス，山積み陳列とチラシ掲載はプラスに推定されている。ダミー変数である山積み陳列とチラシ掲載のオッズ比を計算すると，山積み陳列は $\exp(0.307) = 1.360$，チラシ掲載は $\exp(0.820) = 2.271$ となり，チラシ掲載の方がプロモーション効果が高いと分かる。
...
**データ出典** ：[続・農産物マーケティング活動の手引き - 宮城県公式ウェブサイト](https://www.pref.miyagi.jp/soshiki/res_center/marketing-manual.html)  内の [選択制コンジョイント分析](https://www.pref.miyagi.jp/uploaded/attachment/617762.pdf)  
</br>
...
消費者に製品案の好みを聞く方法としては，「欲しい順番を回答してもらう」，「一番欲しい製品案をひとつ選んでもらう」などがある。しかし，これらの方法は製品案の数が多いと，回答負荷が高くなるため適切な回答を得られない可能性がある。回答負荷を減らす方法として，選択ベース・コンジョイントと呼ばれる手法があり，1人の消費者に対して以下の手順で製品案に対する選好を調査する。

1. 全ての製品案から2つの製品案をランダムに選ぶ
2. 2つの製品案のどちらを買うかまたは「どちらも買わない」を選んでもらう
3. 同じ消費者に1と2を何回か繰り返す

牛乳データの場合は，回答者10人に対して，1人につき1と2を8回繰り返して，3つの選択肢からの選択データを8回分集めている（1つのidにつき，experimentが8回あり，データの行数は10人×8回＝80）。このようにして集めたデータにロジット分析を適用することで，回帰係数から消費者の選考に対する各属性・水準の影響を理解することが出来る。

コンジョイント分析に関しては，以下の参考文献または，データ出典に挙げたリンクを参照されたい。**特に，[選択制コンジョイント分析](https://www.pref.miyagi.jp/uploaded/attachment/617762.pdf) は課題に取り組む前に一読すること。**

**参考文献**：[照井・佐藤（2013）『現代マーケティング・リサーチ』有斐閣](http://www.yuhikaku.co.jp/books/detail/9784641164161)，第8章

### 課題
以下のモデルで多項ロジットモデルを推定し，消費者の牛乳選択における各属性の影響を考察せよ。

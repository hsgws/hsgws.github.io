...
**データ出典**：学部授業で実施したアンケート（2018年度）をもとに作成したシミュレーションデータ
...
# 適合度指標
fitMeasures(result, fit.measures = c("rmsea", "gfi"))
```
...
df <- des %>% 
  select(mean, sd) %>% 
  mutate(ceil = mean + sd, floor = mean - sd)
```
...
CE_alpha <- alpha(data_cs[, 2:4]) # データの2:4列目のCEの質問を指定（以下同様）
PQ_alpha <- alpha(data_cs[, 5:7])
CS_alpha <- alpha(data_cs[, 8:10])

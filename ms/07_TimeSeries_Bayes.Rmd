...
data_tea$day <- weekdays(data_tea$date)   # 曜日列の追加（パラメータ推定には不要）
...
# 当てはめ値と平滑化された状態
data.frame(result$muhat, result$alphahat)
...
##### 平滑化状態（最初の10日分のみ表示）
```{r echo=FALSE}
data.frame(result$muhat, result$alphahat) %>% as.matrix() %>% head(10)
```

...
- オフィス街のコンビニでの販売データ
- 91日間のデータを訓練期間（84日間）と予測期間（7日間）に分割して，前半の訓練期間でパラメータを推定，後半の予測期間で予測力の検証を行う
...
#### 観測モデル
$$
y_t = \alpha_t + s_t + \beta x_t + \varepsilon_t; \qquad  \varepsilon_t \sim N(0, H)
$$

#### システムモデル
$$
\alpha_t = \alpha_{t-1} + v_t; \qquad  v_t \sim N(0, Q_1) \\
s_t = -\sum_{j=1}^6 s_{t-j} + \delta_t; \qquad  \delta_t \sim N(0, Q_2)
$$
...
# install.packages("ggfortify") # 結果図示用のパッケージ
...
# 訓練データと予測データへのデータ分割
train_data <- data_tea[1:84,] # 訓練データ（2015/04/01 ~ 2015/06/23，84日間）
test_data <- data_tea[85:91,] # 予測データ（2015/06/24 ~ 2015/06/30，7日間）
...
システムモデルの誤差項の分散は対角成分の左上からトレンド成分 $\alpha_t$ の分散 $Q_1$，曜日周期成分 $s_t$ の分散 $Q_2$
...
# 予測用データの作成
preddata <- SSModel(
  rep(NA, 7) ~ SSMtrend(1, Q = NA) + SSMseasonal(7, sea.type = "dummy", Q = NA) + holiday,
  H = NA, 
  data = test_data
)

# fitSSM で推定したパラメータの代入
preddata$H <- fit$model$H    # 観測モデルの分散
preddata$Q <- fit$model$Q    # システムモデルの分散

# 予測の実行
pred <- predict(fit$model, n.ahead = 7, newdata = preddata, interval = "prediction", level = 0.95)
...
#### 観測モデル
$$
y_t = \alpha_t + \sum_{j=1}^4 \beta_{j,t} x_{j,t} + \varepsilon_t; \qquad  \varepsilon_t \sim N(0, \sigma^2_y)
$$

#### システムモデル
$$
\alpha_t = \alpha_{t-1} + v_t; \qquad  v_t \sim N(0, \sigma^2_\alpha) \\
\beta_{j,t} = \beta_{j,t-1} + \eta_{j,t}; \qquad  \eta_{j,t} \sim N(0, \sigma^2_j)
$$
...
```{r, results='hide', message=FALSE, fig.keep='none'}
# パッケージのインストール（初回のみ実行）
# install.packages("bayesm")  # 状態空間モデルのパッケージ
# install.packages("ggfortify") # 結果図示用のパッケージ
...
hokkaido %>% 
  mutate(date = ym(date)) %>% 
  ggplot(mapping = aes(x = date, y = num/1000)) +
  geom_line() +
...
どちらのモデルの予測精度が高いか予測期間データのRMSEで比較せよ

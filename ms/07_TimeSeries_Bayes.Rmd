...
- オフィス街のコンビニでの販売データ
- 13週（91日）のデータを訓練期間12週（84日）と予測期間1週（7日間）に分割して，前半の訓練期間でパラメータを推定，後半の予測期間で予測力の検証を行う
...
# 予測の実行
pred <- predict(fit$model, n.ahead = 7, newdata = newdata, interval = "prediction", level = 0.95)
...
# （参考）予測期間の実績値と予測値の図示
test_data <- cbind(test_data, model1_pred)

...
# 回帰分析の実行
result <- lm(log(PI_A) ~ log(PriceIndex_A) + log(PriceIndex_B) + Display_A + Display_B, data = sec3_reg)
...
### 目的変数の予測*
目的変数を対数変換したモデルで予測を行うと，予測結果も対数変換された値で出力され，95％区間の解釈が難しくなるため，以降では対数変換を行わない原数値を目的・説明変数として利用する回帰モデルを扱う。

```{r, results='hide'}
# 原数値モデルの推定
result_raw <- lm(PI_A ~ PriceIndex_A + PriceIndex_B + Display_A + Display_B, data = sec3_reg)
...
# 予測の実行
prediction <- predict(result_raw, newdata = prediction_data, interval = "prediction")
...
### 曜日ダミーの追加*
以降も対数変換を行わない原数値を目的・説明変数として利用する回帰モデルを扱う。
...
# 回帰分析の実行
result_days <- lm(
  PI_A ~ PriceIndex_A + PriceIndex_B + Display_A + Display_B + days,
  data = sec3_reg
)

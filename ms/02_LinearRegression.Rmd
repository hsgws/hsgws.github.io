...
PriceIndex_Aの係数 $\beta_1$ の符号は期待される通りマイナスで推定されており，p値が5％以下のため，5％有意水準で有意と判断される。目的変数と説明変数が両対数のため，係数は価格弾力性を意味し，「価格掛率が1％増えると，点数PIは-5.040％変化する」と解釈できる。

PriceIndex_Bの係数 $\beta_2$ とDisplay_Aの係数 $\beta_3$ も期待通りの符号に推定されており，5％有意水準でも有意である。一方，Display_Bの係数 $\beta_4$ の係数は，5％有意水準で有意と判断されず，目的変数に対して影響していないことが分かる。
...
ダミー変数は {0,1} でダミー変数列を作成する以外にも，文字でダミー変数列を作成することもできる。

...
⇒ [質問項目（50問）と人種の詳細](06_Clustering_bigfive.html)
...
apply(kakei2023, 2, mean) # 各列の平均
apply(kakei2023, 2, sd)   # 各列の標準偏差

# 各列の標準化
kakei2023s <- scale(kakei2023)

apply(kakei2023s, 2, mean)  # 標準化後の各列の平均
apply(kakei2023s, 2, sd)    # 標準化後の各列の標準偏差
...
**`apply(x, margin, fun)`**：データの行または列ごとに関数を適用

- **`x`**：データ
- **`margin`**：関数の適用方向 ⇒ 1 = 行（横方向），2 = 列（縦方向）
- **`fun`**：適用したい関数 ⇒ 平均なら `mean`，標準偏差なら `sd` など

**`scale(x)`**：データの標準化

- **`x`**：データ
- `scale` 関数は各列（縦方向）にデータを標準化
...
### データ
品目別1世帯当たり年間支出金額（都道府県庁所在地） ⇒ 分析例②-1と同一データ

...
- ATTは $\beta = 1.018$ で有意に推定されており，「プロモーションメールを配信した顧客（介入群）におけるプロモーションメールの効果は1.018ドル」と判断できる
  - 介入群はプロモーションメールが配信されたことで購入金額が平均的には1.018ドル増えた
- ATTとATEは異なることに注意
...
- 現実のデータ観測ではバイアス発生の全ての原因を特定することは困難であり，また介入効果推定のために必要な共変量が観測されているかは判断できないないが，そのような場合でも傾向スコア分析およびIPW推定量は介入効果を推定する上で望ましい性質がある（詳細は以下の参考文献の10～12章を参照）

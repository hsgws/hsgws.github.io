...
# マップ上の単語数を減らした図の作成
library(factoextra)
fviz_ca_biplot(result, select.row = list(contrib = 60)) # 単語数を60個に制限
```
...
### データ
**あるタブレット製品のAmazonレビュー（data_tablet_lda.csv）**
...
write.csv(df, "data/data_tablet.csv")
```
</div></details>
...
- **`k`**：トピック数
- **`control`**：オプション ⇒ `seed` の指定で分析繰り返しても同じ出力を取得可能
...
### データ
**スマートウォッチ8製品のAmazonレビュー（data_tablet_lda.csv）**
...
write.csv(df, "data/data_smartwatch.csv")
```
</div></details>
...
data_tablet <- read.csv("data/data_smartwatch.csv", row.names = 1)
result <- CA(data_tablet)

library(factoextra)
result <- CA(data_tablet, graph = FALSE)

...
recency（最終購買からの経過月数）の係数は符号がマイナスに推定されており，最終購買からの経過月数が長いとすでに顧客としては離脱してしまっている可能性が高いため，期待されるとおりの結果である。history（昨年の購入金額）も同様に期待通りの結果で，係数は符号がプラスに推定されており，購買金額の高い優良顧客はプロモーションメールへの反応が高いことが分かる。
...
<!-- | experiment        | 質問番号（1～8）                          | -->
<!-- | choice            | 選択結果                                    | -->
<!-- | int.A ~ int.C     | 切片（int.A = 0，int.B = 0，int.C = 1） ⇒ 本課題では利用しない  | -->
...
<!-- **データ出典** ：[続・農産物マーケティング活動の手引き - 宮城県公式ウェブサイト](https://www.pref.miyagi.jp/soshiki/res_center/marketing-manual.html)  内の [選択型コンジョイント分析](https://www.pref.miyagi.jp/documents/20321/617762.pdf)   -->
<!-- </br> -->
...
<!-- 牛乳データでは，4属性とそれぞれの水準数を単純に掛け合わせると，2×2×2×4＝32通りの製品案が作成される。 -->
<!-- しかし，直行表を使うことによって，製品案を32通りから8通りまで減らすことができる。 -->
...
<!-- **目的変数**：選択結果 (choice)   -->
<!-- **説明変数**：HACCPラベル (haccp)，エコラベルの有無 (eco)，賞味期限 (bb)，1リットルあたり価格 (price) -->
...
<!-- # 多項ロジットモデルの推定 -->
<!-- result <- mlogit(choice ~ 0 + haccp + eco + bb + price, data = milkdata) -->

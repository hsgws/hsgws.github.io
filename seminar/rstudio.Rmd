...
以降では Project フォルダ内に data フォルダを作成し，そこにデータ（data.csv）が保存されていることを前提とする。
...
または，Rstudio の右下ペインの「Files」に Project フォルダのファイル一覧が表示されるので，それをクリックして開くことができる。
...
data.csv の一部
```{r echo=FALSE}
head(read.csv("data/data.csv"))
```

<br />
「(データ名)\$(列名)」でデータ内の特定の列を指定できる。`mydata$sales` は mydata の sales 列。
...
#### R Notebook と R Markdown
R Script はプログラムしか保存できないが，R Notebook や R Markdown を利用すると，プログラムとその分析結果の出力を HTML や PDF 
で保存できる。
詳細は末尾の参考サイト・書籍の2，3などを参照。

[R Notebook で作成した HTML ファイルの例 (https://hsgws.github.io/seminar/sample_code.nb.html)](https://hsgws.github.io/seminar/sample_code.nb.html)
...
3. [浅野・中村（2018）『はじめてのRStudio エラーメッセージなんかこわくない』オーム社](https://www.ohmsha.co.jp/book/9784274222931/) ⇒ 図書館の電子ブック KinoDen で閲覧可能
4. [松村他 (2021) 『改訂2版 Rユーザのための RStudio［実践］入門』技術評論社](https://gihyo.jp/book/2021/978-4-297-12170-9)
...
# 読み込んだデータが文字化けする場合や実行時に「不正なマルチバイト文字があります」のエラーが表示される場合は「fileEncoding = "shift-jis"」を引数に追加
sales.price.promo <- read.table("data/sales-price-promo.txt", header = T, sep = "", 
...
```{r}
library(ggplot2)
ggplot(data = sales.price.promo, 
       mapping = aes(x = 価格, y = 売上, group = factor(販促), colour = factor(販促))) + 
  geom_point()
```

...
## 1. RとRStudioのインストール
RStudio のダウンロードページ [RStudio (https://posit.co/download/rstudio-desktop/)](https://posit.co/download/rstudio-desktop/) にアクセスして，R と RStudio を PC にインストール。

- R：「1: Install R」の「DOWNLOAD AND INSTALL R」を進み，"base"からインストーラーをダウンロード
- RStodio：「2: Install RStudio」からインストーラーをダウンロード
...
## 2. Project の作成
データなどのファイル管理のため，RStudio を開いて以下の手順で授業用の Project を作成する。
...
このファイルを開いて Rstudio を起動すれば，データを読み込む際のパス（保存場所）指定などを簡略化できる。  

今後のために，Project フォルダをエクスプローラーにピン留めするか，デスクトップにショートカットを作成することを推奨。
...
以下のようなエラーが表示される場合は，`read.csv` の引数の場所（`"data/sample_data.csv"`）にデータが保存されてないので，パスが正しいか確認すること。
...
1. エディタ上部の「Source」横の下矢印 → 「Source with Echo」をクリック（またはキーボードで Ctrl + Shift + Enter）
...
## 5. その他
### Posit Cloud
共用パソコンやタブレットなどを利用しており，R と RStudio がインストールできない場合は [Posit Cloud](https://posit.cloud/) でブラウザ上で RStudio を利用できる。
ただし，無料アカウントの場合は利用可能計算時間（月25時間）や利用可能メモリ（1GB）などの制約がある。登録方法や使い方などは以下の参考リンクを参照。

参考リンク（2026/04/14アクセス）

- [【インストール不要】WebでRStudioを使える「Posit Cloud」ー無料枠やデータ入出力の解説ー | Hira Labo](https://hira-labo.com/archives/2570)
- [Posit Cloud (旧RStudio Cloud)とは](https://shunichinomura.github.io/cloud.html)
...
その場合は出力されたエラー文章をAIやウェブで検索すると多くの場合は解決策が見つかる。
